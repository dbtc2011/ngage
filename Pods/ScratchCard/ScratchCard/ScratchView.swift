...
//        let image = processPixels(image: UIImage(named: couponImage)!)
        var image = UIImage(named: couponImage)
        if image != nil {
            let text = UserDefaults.standard.string(forKey: "scratch_point")
            image = textToImage(drawText: NSString(format: "\(text!)" as NSString), inImage: image!, atPoint: CGPoint(x: 120, y: 100))
