...
class TaskTableViewCell: UITableViewCell {
  ...
  func setupUsing(task : TaskModel, color: UIColor) {
    ...
    case TaskStatus.done.rawValue:
      ...
      let status = (UserDefaults.standard.value(forKey: "rewarded_\(task.missionCode)_\(task.code)") == nil)
      leftIcon.backgroundColor = color
      ...
      if (task.type == 5 || task.type == 18) && status {
        rightIcon.image = UIImage(named: "ic_reward_disabled")
      }
    ...
    var rewardValue = task.reward
    if task.type == 18 {
      rewardValue = task.isReward
    }
    labelReward.text = getRewardWithUOM(reward: rewardValue)
