...
class MultipleChoiceTaskViewController: UIViewController {
  ...
  func playButtonSound(correct: Bool, completion: @escaping() -> Void) {
    var fileName = "quiz_correct"
    if !correct {
      fileName = "quiz_wrong"
    }
        ...
        self.getDataFromUrl(url: imageUrl, completion: { (data, response, error) in
          ...
          DispatchQueue.main.async {
            if let image = UIImage(data: data) {
              self.questionView!.imageLogo.image = image
  ...
  @IBAction func answerButtonClicked(_ sender: UIButton) {
    let isCorrect = (sender.tag == correctTag)
    ...
    playButtonSound(correct: isCorrect) {
      if self.task.type == 8 {
        if self.player == nil {
        ...
        if self.timeLimit != nil {
          self.timeLimit!.invalidate()
        }
        self.playerView!.button.isHidden = false
        self.player = nil
      } else if self.task.type == 18 && !isCorrect {
        self.cutTask()
        return
      }
      ...
      self.answers = self.answers + (sender.titleLabel?.text ?? "")
      if isCorrect {
