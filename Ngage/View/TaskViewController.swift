...
class TaskViewController: MainViewController {
  ...
  var customReward: CustomRewardView?
  ...
  func showModalForType18(totalPoints: String) {
    customView.isHidden = false
    let modalView = Bundle.main.loadNibNamed("SuccessWithVerifcationView", owner: self, options: nil)?.first as! SuccessWithVerifcationView
    modalView.config(for: self.selectedTask, totalPoint: Int(totalPoints)!)
    modalView.delegate = self
    modalView.bounds = customView.bounds
    modalView.frame.origin = CGPoint(x: 0, y: 0)
    customView.addSubview(modalView)
  ...
  func didFinishTask(task: TaskModel) {
    UserDefaults.standard.set(true, forKey: keyShouldUpdate)
    ...
    case 18:
      var totalPoint = 0
      if Util.pendingReward(for: self.selectedTask) {
        totalPoint = Int(user.points)!
      } else {
        totalPoint = Int(user.points)! + Int(task.isReward)!
      }
      if quizAnswers == nil {
        quizAnswers = ""
      }
      if quizCorrectAnswers == nil {
        quizCorrectAnswers = ""
      }
      submitTask(missionID: "\(mission.code)", taskID: "\(task.code)", tasktype: "\(task.type)", FBID: user.facebookId, ContentID: "0", SubContentID: "1", Answer: quizAnswers, CorrectAnswer: quizCorrectAnswers, WatchType: "", WatchTime: "", DeviceID: user.deviceID, TaskStatus: "2", Current_Points: "\(totalPoint)", Points: task.isReward, Prev_Points: user.points)
      quizCorrectAnswers = ""
      quizAnswers = ""
  ...
  func showErrorForWrongAnswer(`with` message: String, tag: Int) {
    if customReward != nil {
      customReward?.removeFromSuperview()
      customReward = nil
    }
    user.points = UserModel().mainUser().points
    customReward = Bundle.main.loadNibNamed("CustomRewardView", owner: self, options: nil)?.first as? CustomRewardView
    customReward!.tag = tag
    customReward!.delegate = self
    customReward?.config(with: message)
    customReward!.bounds = UIScreen.main.bounds
    customReward!.frame.origin = CGPoint(x: 0, y: 0)
    if let window = UIApplication.shared.keyWindow {
      window.addSubview(customReward!)
    }
  }
  ...
  func triggerTask() {
    showSpinner()
    print("Should trigger api")
    RegisterService.triggerTask(FBID: user.facebookId, MID: mission.code, TID: selectedTask.code) { (result, error) in
      self.hideSpinner()
      if let result = result {
        if let statusDec = result["StatusDesc"].int {
          var message = ""
          var tag = 1
          switch statusDec {
          case 1:
            tag = 1
            message = "You only have 1 remaining attempt to get all the answers correctly."
          default:
            tag = 2
            message = "Sorry!\nYou failed to do this task with the given number of attempt/s."
          }
          self.showErrorForWrongAnswer(with: message, tag: tag)
        }
        ...
        if let statusCode = result!["StatusCode"].int {
          if statusCode == 2 {
            if let points = result!["Points"].int {
              ...
              case 18:
                self.taskPoint = self.selectedTask.isReward
              default :
              ...
              CoreDataManager.sharedInstance.updateUserPoints(withPoints: "\(points)", completionHandler: { (coreResult) in
                DispatchQueue.main.async {
                  ...
                  if self.selectedTask.type == 4 {
                    ...
                    if self.selectedTask.instructions.contains("share ") {
                      self.showModalForPendingReward(message: "", type: "referral")
                    } else {
                  ...
                  } else if self.selectedTask.type == 18 {
                    self.showModalForType18(totalPoints: "\(points)")
                  } else {
...
extension TaskViewController: CustomRewardViewDelegate {
  func customRewardDidTapButton() {
    
    let shouldCompleteTask = (customReward!.tag == 2)
    if customReward != nil {
      customReward!.removeFromSuperview()
    }
