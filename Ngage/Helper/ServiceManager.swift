...
final class RegisterService: RequestManager {
  ...
  class func checkMissionAvailability(FBID: String, MissionID: Int, success: @escaping CompletionBlock) {
    ...
    perform(task: .checkMission(parameter)) { (result, error) in
      print("Task result = \(String(describing: result))")
      success(result, error)
    }
  }
  
  class func triggerTask(FBID: String, MID: Int, TID: Int, success: @escaping CompletionBlock) {
    let parameter = ["FBID": FBID, "MID": MID, "TID": TID] as [String : Any]
    perform(task: .trigger(parameter)) { (result, error) in
      print("Task result = \(String(describing: result))")
      success(result, error)
    }
  }
